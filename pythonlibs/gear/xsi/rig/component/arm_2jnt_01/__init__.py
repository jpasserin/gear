...
class Component(MainComponent):
    ...
    def addObjects(self):
        ...
        # FK Controlers ------------------------------------
        self.fk_cns = pri.addNull(self.root, self.getName("fk_cns"), self.root.Kinematics.Global.Transform, self.size*.02)
        self.addToGroup(self.fk_cns, "hidden")

        t = tra.getTransformLookingAt(self.guide.apos[0], self.guide.apos[1], self.normal, "xz", self.negate)
        self.fk0_ctl = self.addCtl(self.fk_cns, "fk0_ctl", t, self.color_fk, "cube", h=self.size*.1, w=1, d=self.size*.1, po=XSIMath.CreateVector3(.5*self.n_factor,0,0))
